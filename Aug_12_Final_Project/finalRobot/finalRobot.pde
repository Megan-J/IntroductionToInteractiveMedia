/*
 ...
 Initial Completion: August 9, 2020
 ...
 used the information found from the sound libraries in Processing. The background music is Memories Renewed by Ketsa
...
void setup() {
  ...
  //calls the welcome text and images
  w = new welcome();
...
void draw() {
  ...
  if (gamestate == 0) {
    ...
    //run the display function in the Welcome class
    w.display();
    ...
    for (int i = 0; i < (circles.length); i++) {
      ...
      //changes the circles' color based on its number in the array
      if (i % 2 == 0) {
        fill(255, potentiometer, 178, potentiometer);
      } else {
        fill(172, 255, potentiometer, potentiometer);
      }
...
void soundFrequency() {
  ...
  if (potentiometer >= 175) {
    fill(255, 56, 112, 200);
  } else if (potentiometer < 175) {
    fill(215, 56, 255, 200);
  }

  noStroke();

  //draws the bottom bands across the screen
  ...
  //reverses the information in the fft.spectrum array
  for (int i = 0; i < fft.spectrum.length/2; i++) {

    float temp = fft.spectrum[i];
    fft.spectrum[i] = fft.spectrum[fft.spectrum.length - i - 1];
    fft.spectrum[fft.spectrum.length - i - 1] = temp;
  }

  //println("after: ");
  //println(fft.spectrum);

  //draws the top bands across the screen
  for (int i = 0; i < bands; i++) {
    ...
    // Draw the rectangles, adjust their height using the scale factor
    rect(i*barWidth, 0, barWidth, sum[i]*200*scale, 5);
  }
}
