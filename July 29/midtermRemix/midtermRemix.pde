/* 
 ...
 Updated July 28, 2020 for another assignment to use Processing with Arduino. The new audio added is 
 Moonrise by Chad Crounch.
...
SoundFile step;
SoundFile point;
SoundFile backgroundMusic;  //added background music while playing the game 7/29

//-----------------------Modifications for 7/29 Assignment-----------------------//

//importing the serial library and initializing a variable for the 
//port
import processing.serial.*;
Serial port;

//values that the Ultrasonic Sensor sends to Arduino and sends to processing
float physicalDistance = 0;  

//booleans for different playing difficulties
boolean easy = false;
boolean hard = false;
...
void setup() {
  ...
  //connects the to the port the Arduino board is using
  println(Serial.list());
  ...
  backgroundMusic.loop();  //loops background music throughout game play
...
void draw() {
  ...
  //7/29 addition: declares the different difficulties based on the 
  //Ultrasonic Sensor's values
  playerMode();
...
void mouseClicked() {
  ...
  if (gameOver == true) {
    ...
    redraw();
    backgroundMusic.play();  //play the background music again after restarting
  }
}
...
void playerMode() {
  ...
  //if object is 0 to 6 cm away from the Ultrasonic Sensor
  if (physicalDistance >= 0 && physicalDistance <= 6) {

    easy = true;  //boolean is set to true and player wants
    //to play on easy mode
  } else {  //if not then boolean is set to false
    easy = false;
  }


  //if object is 7 to 14 cm away from the Ultrasonic Sensor
  if (physicalDistance >= 7 && physicalDistance <= 14) {

    hard = true;  //boolean is set to true and player wants
    //to play on hard mode
  } else {  //if not the certain distance then player is not playing 
    //on hard mode
    hard = false;
  }

  //if object is 18 to 20 cm away from the Ultrasonic Sensor
  if (physicalDistance >= 15 && physicalDistance <= 20) {

    extreme = true;  //boolean is set to true and player wants
    //to play on extreme mode
  } else {  //if not the certain distance then player is not playing on 
    //extreme mode
    extreme = false;
  }
}
...
void serialEvent(Serial port) {
  ...
  //gets the information from the Arduino
  String inString = port.readStringUntil('\n');

  if (inString != null) {  //if the inString reads the information

    //removes any whitespace
    inString = trim(inString);

    //converts the information into a float variable
    physicalDistance = float(inString);

    //prints the information so I know what the values of 
    //certain distances were
    println(physicalDistance);
  }
}
